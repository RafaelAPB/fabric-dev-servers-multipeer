...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org4.example.com/msp" peer0.org5.example.com peer channel fetch config -o orderer0.example.com:7050 -c composerchannel
...
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org4.example.com/msp" peer0.org5.example.com peer channel join -b composerchannel_config.block